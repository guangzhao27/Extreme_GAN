...
tt = torch.load('data/real.pt')
dataloader = DataLoader(NWSDataset(), batch_size=16, shuffle=True)
...
for epoch in range(1000):
    ...
    if (epoch + 1) % 10 == 0:   
        with torch.no_grad():
            ...
            G.train()
G.eval()
fakeSamples = G(Variable(torch.randn(int(254/0.75), latentdim, 1, 1)).cuda())
