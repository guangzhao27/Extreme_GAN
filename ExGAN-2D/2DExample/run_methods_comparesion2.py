...
sigma=0.1
bin_width = 0.002
tail_factor_list = [2.3, 3, 3.7]
MethodSet = ('Original', 'SingleGaussian', 'GMM', 'Dshift', 'ExGAN')
# MethodSet = ('SingleGaussian', 'GMM', 'Dshift',)
...
for tail_factor in tail_factor_list:
    ...
    for _ in range(round_num):
        ...
        if "Original" in MethodSet and tail_factor<3.5:
            start = time()
            ...
            #second method
        if "SingleGaussian" in MethodSet:
            start = time()
        ...
        if "ExGAN" in MethodSet and tail_factor<3.5:
            sample_total = 0
            start = time()
            T = 0
            ...
            while T < 50000:
                if T >= flag:
                    ...
                    flag += 10000
                sample_size = 100000
                lf = torch.FloatTensor(rv.rvs(sample_size)).view(sample_size, -1).to(device)
                lf = lf[lf>threshold].view(-1, 1)
                
                sample_total += len(lf)
                
                z = torch.randn(len(lf), 2).to(device)
