...
def plot_multi_days_scenarios2(quantiles: np.array, scenarios: np.array, y_true: np.array, dir_path: str, name: str, n_s: int,
                              ylim: float = 1):
    """
    Plot 4 days with subplots (2, 1).
    :param quantiles: (n_periods, n_s).
    ...
    T = 1
    FONTSIZE = 10
    x_index = [i for i in range(1, T * 24 + 1)]
    fig, axs = plt.subplots(2, 1, sharex='col', sharey='row', gridspec_kw={'hspace': 0, 'wspace': 0})
    axs[0].plot(x_index, scenarios[:T * 24, :n_s], color='gray', linewidth=1, alpha=0.5)
    # axs[0].plot(x_index, quantiles[:T * 24, 9], color='b', linewidth=2, label='10 %')
    axs[0].plot(x_index, quantiles[:T * 24, 49], color='k', linewidth=2, label='50 %')
    # axs[0].plot(x_index, quantiles[:T * 24, 89], color='g', linewidth=2, label='90 %')
    axs[1].plot(x_index, y_true.reshape(24, -1), color='r', linewidth=2, label='true')
    # axs.set_ylim(0, ylim)
    axs[0].tick_params(axis='both', labelsize=FONTSIZE)
...
def quantiles_and_evaluation(dir_path: str, s_VS: np.array, s_TEST: np.array, N_q: int, df_y_VS: pd.DataFrame,
    ...
    if not test_type:
        ...
        #for z in range(nb_zones):
        z = 9
        plot_multi_days_scenarios2(quantiles=q_TEST[24 * nb_days_per_zone * 0:24 * nb_days_per_zone * (z + 1), :],
                                scenarios=s_TEST[24 * nb_days_per_zone * 0:24 * nb_days_per_zone * (z + 1), :],
                                y_true=df_y_TEST.values.reshape(-1)[
                                24 * nb_days_per_zone * 0:24 * nb_days_per_zone * (z + 1)], dir_path=dir_path,
                                name='TEST_zone_' + str(z) + '_' + name, n_s=50, ylim=1)
    # --------------------------------------------------------------------------------------------------------------
    # CRPS
    # --------------------------------------------------------------------------------------------------------------
