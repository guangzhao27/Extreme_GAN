...
class Generator_linear(nn.Module):
    ...
    def __init__(self, test_type=False, **kwargs):
        ...
        if not test_type:
            ...
            l_gen_net = [self.latent_s] + [kwargs['gen_w']] * kwargs['gen_l'] + [self.in_size]
    ...
    def sample(self, n_s=1, x_cond:np.array=None):
        ...
        if not self.test_type:
            ...
            scenarios = self.gen(z).view(n_s, -1).cpu().detach().numpy()
...
class Discriminator_wassertein(nn.Module):
    ...
    def __init__(self, test_type=False, **kwargs):
        ...
        if not test_type:
            ...
            l_dis_net = [self.in_size] + [kwargs['gen_w']] * kwargs['gen_l'] + [1]
        self.test_type = test_type
    ...
    def loss(self, generated_samples: torch.Tensor, true_samples: torch.Tensor, context: torch.Tensor):
        ...
        if not self.test_type:
            ...
            D_true = self.dis(true_samples)####
            D_generated = self.dis(generated_samples)
            # Compute Discriminator's loss with a gradient penalty to force Lipschitz condition
            gp = self.grad_pen(real=true_samples, samples=generated_samples, context=context)
    ...
    def grad_pen(self, real: torch.tensor, samples: torch.tensor, context: torch.Tensor):
        ...
        if not self.test_type:
            ...
            mixed_score = self.dis(interpolated_sample)
...
def fit_gan_wasserstein(nb_epoch: int, x_LS: np.array, y_LS: np.array, x_VS: np.array, y_VS: np.array, 
                        x_TEST: np.array, y_TEST: np.array, gen, dis, opt_gen, opt_dis, 
                        n_discriminator:int, batch_size:int=100, wdb:bool=False, gpu:bool=True, test_type=False):
    """
    ...
    for epoch in range(nb_epoch):
        ...
        for y_batch, x_batch in zip(np.split(y_LS_shuffled, batch_list), np.split(x_LS_shuffled, batch_list)):
            ...
            if not test_type:
                ...
                loss_D = dis.loss(generated_samples=G_LS_samples, true_samples=y_batch_LS, context=torch.ones_like(x_batch_LS))   
        ...
        loss_D_LS = loss_D_batch / batch_dis_idx # cross entropy value range from 0 to infinity 
        ...
        if not test_type:
            ...
            loss_D_VS = dis.loss(generated_samples=G_VS_samples, true_samples=y_VS_gpu, context=torch.ones_like(x_VS_gpu)).detach()
        # G
        D_VS_gen = dis(input=G_VS_samples, context=x_VS_gpu)
        D_VS_true = dis(input=y_VS_gpu, context=x_VS_gpu)
        loss_G_VS = -torch.mean(D_VS_gen).detach() + torch.mean(D_VS_true).detach()
        ...
        if not test_type:
            ...
            loss_D_TEST = dis.loss(generated_samples=G_TEST_samples, true_samples=y_TEST_gpu, context=torch.ones_like(x_TEST_gpu)).detach()
        ...
        D_TEST_gen = dis(input=G_TEST_samples, context=x_TEST_gpu)
        D_TEST_true = dis(input=y_TEST_gpu, context=x_TEST_gpu)

        loss_G_TEST = -torch.mean(D_TEST_gen).detach() + torch.mean(D_TEST_true).detach()
        ...
        if epoch % 10 == 0:
            print("Epoch {:.0f} Approximate time left : {:2f} min - D LS loss: {:4f} G LS loss: {:4f} D VS loss: {:4f} G VS loss: {:4f}".format(
                epoch, 
                time_tot / (epoch + 1) * (nb_epoch - (epoch + 1)) / 60, 
                loss_D_LS, loss_G_LS, loss_D_VS, loss_G_VS), end="\r", flush=True)
...
def plot_GAN_loss(loss: np.array, nb_days: list, ylim: list, dir_path: str, name: str):
    ...
    plt.figure()
    plt.plot(loss[:, 0], label='D Train')
    
    plt.plot(loss[:, 2], label='D Val')
    
    plt.plot(loss[:, 4], label='D Test')
    plt.plot(loss[:, 1], label='G Train')
    plt.plot(loss[:, 3], label='G Val')
    ...
    plt.title(name+'#Train ' + str(nb_days[0])+' #Val ' + str(nb_days[1])+' #Test ' + str(nb_days[2]))
