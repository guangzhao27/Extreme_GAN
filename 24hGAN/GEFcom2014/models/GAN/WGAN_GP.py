...
import torch
import sys
sys.path.append("/Users/guangzhao/Downloads/github/OE-weather-generative-model")####
...
dis = Discriminator_wassertein(test_type=Test_Type, latent_s=config['latent_s'], cond_in=config['cond_in'], in_size=config['in_size'], gen_w=config['gen_w'], gen_l=config['gen_l'], lambda_gp=config['lambda_gp'], gpu=gpu)
dis.weights_initialize(mean=0.0, std=0.02)
dis.train()
# Instance generator neural network
gen = Generator_linear(test_type=Test_Type, latent_s=config['latent_s'], cond_in=config['cond_in'], in_size=config['in_size'], gen_w=config['gen_w'], gen_l=config['gen_l'], gpu=gpu)
...
loss, gen, dis = fit_gan_wasserstein(nb_epoch=nb_epoch, x_LS=x_LS_scaled, y_LS=y_LS_scaled, x_VS=x_VS_scaled,
                                                y_VS=y_VS_scaled, x_TEST=x_TEST_scaled, y_TEST=y_TEST_scaled, gen=gen, dis=dis, opt_gen=opt_G, 
                                                opt_dis=opt_D, n_discriminator=config['n_discriminator'], gpu=gpu, test_type=Test_Type)
...
quantiles_and_evaluation(dir_path=dir_path, s_VS=s_VS, s_TEST=s_TEST, N_q=N_q, df_y_VS=df_y_VS, 
                         df_y_TEST=df_y_TEST, name=name, ymax_plf=ymax_plf, ylim_crps=ylim_crps, tag=tag, nb_zones=nb_zones, test_type=Test_Type)
